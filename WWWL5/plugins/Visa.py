...
@WWWL5.ar_cmd(pattern="cc(?:\s|$)([\s\S]*)")
async def song2(event):
    song = event.pattern_match.group(1)
    chat = "@SDBB_Bot" # code by t.me/WWWL5
...
@WWWL5.ar_cmd(pattern="فيزا(?:\s|$)([\s\S]*)")
async def song2(event):
    chat = "@SDBB_Bot"
    reply_id_ = await reply_id(event)
...
@WWWL5.ar_cmd(pattern="توليد(?:\s|$)([\s\S]*)")
async def song2(event):
...
@WWWL5.ar_cmd(pattern="ماستر(?:\s|$)([\s\S]*)")
async def song2(event):
...
@WWWL5.ar_cmd(pattern="اماكس(?:\s|$)([\s\S]*)")
async def song2(event):
    card = event.pattern_match.group(1)
    chat = "@SDBB_Bot"
    reply_id_ = await reply_id(event)
